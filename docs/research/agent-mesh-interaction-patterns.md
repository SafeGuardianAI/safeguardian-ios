...
to run inference at all before paying the compute cost. `AgentGateRegistry`
and `BatteryGate` evaluate gates before `provider.generate()` is called,
avoiding the token cost of a post-inference sentinel.
